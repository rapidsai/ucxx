...
class UCXXEndpoint : public UCXXComponent
{
    ...
    {
        auto worker = UCXXEndpoint::getWorker(_parent);
        auto request = tag_msg(worker, _handle, true, buffer, length, tag, callbackFunction, callbackData);
        return createRequest(request);
    }
    ...
    {
        auto worker = UCXXEndpoint::getWorker(_parent);
        auto request = tag_msg(worker, _handle, false, buffer, length, tag, callbackFunction, callbackData);
        return createRequest(request);
    }
