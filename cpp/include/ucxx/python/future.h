...
static int intern_strings(void)
{
    ...
    if (py_ucxx_asyncio_str == NULL) {
        return -1;
    }
    py_ucxx_future_str = PyUnicode_InternFromString("Future");
    if (py_ucxx_future_str == NULL) {
        return -1;
    }
    return 0;
}
...
err:
    if (!PyErr_Occurred())
        PyErr_SetString(PyExc_RuntimeError,
                        "could not initialize UCXX Python C-API.");
    return -1;
}
...
static PyCFunction get_future_method(const char* method_name)
{
    ...
    if (!result)
        PyErr_Format(PyExc_RuntimeError,
                     "Unable to load function pointer for `Future.set_result`.");
...
err:
    PyErr_Format(PyExc_RuntimeError,
                 "Error while setting exception for `asyncio.Future`.");
finish:
