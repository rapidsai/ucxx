...
void tag_recv_multi_frames(std::shared_ptr<void> ucxxBufferRequests)
{
  ...
  ucxx_trace_req("tag_recv_multi_frames request: %p, tag: %lx", bufferRequests.get(), tag);
  ...
  ucxx_trace_req("tag_recv_multi_frames request: %p, tag: %lx, size: %lu, isFilled: %d",
                 bufferRequests.get(),
                 tag,
                 bufferRequests->bufferRequests.size(),
...
void tag_recv_multi_header(std::shared_ptr<void> ucxxBufferRequests)
{
  ...
  ucxx_trace_req(
    "tag_recv_multi_header entering, request: %p, tag: %lx", bufferRequests.get(), tag);
  ...
  ucxx_trace_req("tag_recv_multi_header exiting, request: %p, tag: %lx, empty: %d",
                 bufferRequests.get(),
                 tag,
                 bufferRequests->bufferRequests.empty());
}
...
void tag_recv_multi_callback(std::shared_ptr<void> ucxxBufferRequests)
{
  ...
  ucxx_trace_req("tag_recv_multi_callback request: %p, tag: %lx", bufferRequests.get(), tag);

  if (bufferRequests->bufferRequests.empty()) {
    ucxx_trace_req(
      "tag_recv_multi_callback first header, request: %p, tag: %lx", bufferRequests.get(), tag);
    ...
    ucxx_trace_req(
      "tag_recv_multi_callback request: %p, tag: %lx, "
...
UCXXBufferRequestsPtr tag_recv_multi(std::shared_ptr<UCXXEndpoint> endpoint, ucp_tag_t tag)
{
  ...
  ucxx_trace_req("tag_recv_multi request: %p, tag: %lx", bufferRequests.get(), tag);
...
// {
//     std::cout << "tag_recv_multi_frames_callback: " << *std::static_pointer_cast<int>(data) <<
//     std::endl;
//     // free(data);
// }
...
//         auto bufferRequest = std::make_shared<UCXXBufferRequest>();
//         bufferRequest->stringBuffer = std::make_shared<std::string>(Header::dataSize(), 0);
//         // auto headerRequest = endpoint->tag_recv(headerSerialized.data(),
//         headerSerialized.size(), tag); auto cb_data = std::make_shared<int>(99);
//         bufferRequest->request = endpoint->tag_recv(bufferRequest->stringBuffer->data(),
//                                                     bufferRequest->stringBuffer->size(),
//                                                     //tag, (void*)tag_recv_multi_frames_callback,
//                                                     (void*)cb_data); tag,
//                                                     (void*)tag_recv_multi_frames_callback,
//                                                     std::static_pointer_cast<void>(cb_data));
//                                                     // tag,
//                                                     (void*)tag_recv_multi_frames_callback,
//                                                     std::dynamic_pointer_cast<void>(bufferRequests));
//                                                     // tag);
...
UCXXBufferRequestsPtr tag_send_multi(std::shared_ptr<UCXXEndpoint> endpoint,
