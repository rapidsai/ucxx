...
/**
 * @brief Create a RemoteKeyBuilder from a local memory handle.
 *
 * @return A RemoteKeyBuilder object.
 */
[[nodiscard]] inline RemoteKeyBuilder createRemoteKeyFromMemoryHandle(
  std::shared_ptr<MemoryHandle> memoryHandle)
{
  return RemoteKeyBuilder(std::move(memoryHandle));
}

/**
 * @brief Create a RemoteKeyBuilder from a serialized remote key.
 *
 * @return A RemoteKeyBuilder object.
 */
[[nodiscard]] inline RemoteKeyBuilder createRemoteKeyFromSerialized(
  std::shared_ptr<Endpoint> endpoint, SerializedRemoteKey serializedRemoteKey)
{
  return RemoteKeyBuilder(std::move(endpoint), std::move(serializedRemoteKey));
}
