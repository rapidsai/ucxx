...
/**
 * @brief Create a ListenerBuilder.
 *
 * @return A ListenerBuilder object.
 */
[[nodiscard]] inline ListenerBuilder createListener(std::shared_ptr<Worker> worker,
                                                    uint16_t port,
                                                    ucp_listener_conn_callback_t callback,
                                                    void* callbackArgs)
{
  return ListenerBuilder(std::move(worker), port, callback, callbackArgs);
}
