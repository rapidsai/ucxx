...
/**
 * @brief Create a MemoryHandleBuilder.
 *
 * @param[in] context parent context where memory is mapped.
 * @param[in] size minimum size of the memory allocation.
 * @return A MemoryHandleBuilder object.
 */
[[nodiscard]] inline MemoryHandleBuilder createMemoryHandle(std::shared_ptr<Context> context,
                                                            size_t size)
{
  return MemoryHandleBuilder(std::move(context), size);
}
