...
status=0
if [ -z ${UCXX_ROOT:+PLACEHOLDER} ]; then
    UCXX_BUILD_DIR=$(git rev-parse --show-toplevel 2>&1)/cpp/build
    status=$?
else
    UCXX_BUILD_DIR=${UCXX_ROOT}
fi

if ! [ ${status} -eq 0 ]; then
    if [[ ${UCXX_BUILD_DIR} == *"not a git repository"* ]]; then
        echo "This script must be run inside the cugraph-gnn repository, or the UCXX_ROOT environment variable must be set."
    else
...
DEFAULT_FORMAT_FILE_LOCATIONS=(
  "${UCXX_BUILD_DIR:-${HOME}}/_deps/rapids-cmake-src/cmake-format-rapids-cmake.json"
)
