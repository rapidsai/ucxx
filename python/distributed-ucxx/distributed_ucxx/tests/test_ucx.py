...
from distributed_ucxx import ucxx  # noqa: E402
...
def test_registered(ucxx_loop):
    assert "ucxx" in backends
    backend = get_backend("ucxx")
    assert isinstance(backend, ucxx.UCXXBackend)


async def get_comm_pair(
    listen_addr="ucxx://" + HOST, listen_args=None, connect_args=None, **kwargs
):
...
async def test_ucxx_specific(ucxx_loop):
    ...
    listener = await ucxx.UCXXListener(address, handle_comm)
...
async def test_simple(
    ...
    async with LocalCluster(protocol="ucxx", asynchronous=True) as cluster:
        async with Client(cluster, asynchronous=True) as client:
